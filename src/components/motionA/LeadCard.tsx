...
export default function LeadCard({ lead, selected, onClick }: LeadCardProps) {
  return (
    <button
      onClick={onClick}
      className={`relative flex w-full gap-3 border-b border-border px-3 py-3 text-left transition-colors ${
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-1 flex items-start justify-between gap-2">
          <span className="truncate text-[14px] font-bold text-navy">{lead.contact_name}</span>
          <TierBadge tier={lead.tier} />
        </div>
        <p className="micro-label mb-2 truncate text-text-muted">{lead.school_name}</p>
        <div className="flex items-center justify-between">
          ...
          {lead.ac_deal_value != null && (
            <span className="text-body-sm font-bold text-navy">{formatZAR(lead.ac_deal_value)}</span>
          )}
        </div>
      </div>
    </button>
